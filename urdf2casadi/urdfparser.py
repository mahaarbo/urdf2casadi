...
class URDFparser(object):
	...
	def get_joint_info(self, root, tip):
		"""Using an URDF to extract joint information, i.e list of joints, actuated names and upper and lower limits."""
	...
	def _model_calculation(self, root, tip, q):
		"""Calculates and returns model information needed in the dynamics algorithms caluculations, i.e transforms, joint space and inertia."""
		if self.robot_desc is None:
		    raise ValueError('Robot description not loaded from urdf')
	...
	def _apply_external_forces(self, external_f, f, i_X_p):
		"""Internal function for applying external forces in dynamics algorithms caluculations."""
	...
	def get_forward_dynamics_crba(self, root, tip, gravity = None, f_ext = None):
			"""Returns the forward dynamics as a casadi function by solving the Lagrangian eq. of motion.
			OBS! Not appropriate for robots with a high number of dof -> use get_forward_dynamics_aba()."""
	...
	def get_forward_dynamics_aba(self, root, tip, gravity = None, f_ext = None):
		"""Returns the forward dynamics as a casadi function using the articulated body algorithm."""
