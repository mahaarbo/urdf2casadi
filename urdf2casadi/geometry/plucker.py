...
def spatial_inertia_matrix_Ic(ixx, ixy, ixz, iyy, iyz, izz, mass):
    """Returns the 6x6 spatial inertia matrix expressed at the center of mass."""
    Ic = np.zeros([6, 6])
...
def spatial_force_transform(R, r):
    """Returns the spatial force transform from a 3x3 rotation matrix and a 3x1 displacement vector."""
    X = cs.SX.zeros(6,6)
    X[:3, :3] = R.T
    X[3:, 3:] = R.T
...
def spatial_transform_BA(R, r):
    """Returns the inverse spatial motion transform from a 3x3 rotation matrix and a 3x1 displacement vector."""
    X = cs.SX.zeros(6,6)
    X[:3, :3] = R.T
    X[3:, 3:] = R.T
    X[3:, :3] = cs.mtimes(cs.skew(r), R.T)
...
def XJT_revolute_BA(xyz, rpy, axis, qi):
    """Returns the spatial transform from parent link to child link with a revolute connecting joint."""
    T = tm.revolute(xyz,rpy,axis,qi)
    rotation_matrix = T[:3,:3]
    displacement = T[:3, 3]
...
def XJT_prismatic_BA(xyz, rpy, axis, qi):
    """Returns the spatial transform from parent link to child link with a prismatic connecting joint."""
    T = tm.prismatic(xyz,rpy,axis,qi)
    rotation_matrix = T[:3,:3]
    displacement = T[:3, 3]
    return spatial_transform_BA(rotation_matrix, displacement)
